...
function teamAccolades(name){
  ...
  return `<section class="stat accolades"><h3>Team Accolades</h3>
    ...
    ${hasAwards ? `<button class="pname" data-tawards="${esc(name)}">Player Awards →</button>` : ''}
  </section>`;
}
...
const NON_PLAYER_AWARDS = new Set(['Team of the Year', 'Game Of The Year', 'Sox Trophy']);
/* every individual-player award a franchise's players have won, resolved
   through the same team-abbreviation machinery tnick() uses on the main
   Awards page — paired positionally with the winner list on the old
   ...
   1:1 (a few of those old rows have mismatched counts) */
