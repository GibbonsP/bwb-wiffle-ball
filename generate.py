...
.svdot{position:absolute;top:50%;width:18px;height:18px;border-radius:50%;
  ...
  transition:left .7s ease,background-color .7s ease,opacity .7s ease}
...
.pfpill{display:inline-block;min-width:2.2em;padding:2px 9px;border-radius:999px;
  font-family:"IBM Plex Mono",monospace;font-size:.78rem;font-weight:700;color:#fff;
...
.gpoolreveal summary::-webkit-details-marker{display:none}
.gpoolreveal summary:hover{color:var(--accent)}
.gpoollist{margin:6px 0 0;padding-left:1.1em;font-size:.72rem;max-height:200px;overflow-y:auto}
.gpoollist li{margin:2px 0}
...
<footer class="sitefoot">
  <div class="footsocial">
    ...
    <a href="https://prowiffleball.com/" target="_blank" rel="noopener" aria-label="BWB Wiffleball on ProWiffleball">
...
/* Chances handled = PO+A+E, not the separately-recorded TC field — checked
   against the real data and found 8 real season rows (out of 122) where TC
   didn't actually equal PO+A+E, which let fielding % come out above 1.000
   (Parker Gibbons' 2025: 37 PO + 16 A over a recorded TC of 51, i.e. 53/51).
   Deriving the denominator from PO+A+E instead makes a >1.000 percentage
   mathematically impossible, regardless of what TC itself was recorded as. */
const fld = d => (d.PO+d.A+d.E) ? (d.PO+d.A)/(d.PO+d.A+d.E) : NaN;
...
function teamHistoryChips(pl){
  ...
  return `<span class="tmchips">${teams.map(t=>{
    ...
    return `<button class="pname tmchip" data-t="${esc(t.team)}" title="${esc(label)} · ${yrs}">${logo
      ? logoIcon(logo, label, 'llogo llogo-sm') : esc(TEAMS[t.team]?TEAMS[t.team].nick:t.team)}</button>`;
  }).join('')}</span>`;
}
...
function renderHome(){
  ...
  const ll=(t,items,fmt,low)=>{
    ...
    return `<div class="llist"><h4>${t}</h4><ol>${items.map(it=>{
      const logo = logoIcon(it.logo2 || it.logo, '');
  ...
  app.innerHTML = `
    ...
    <div class="followgrid">
      ...
      <a class="followcard pw" href="https://prowiffleball.com/" target="_blank" rel="noopener">
...
/* maps a park-factor-style number (100 = neutral) onto the same blue→grey→red
   scale svColor already uses for percentiles — a factor 30 points from 100
   in either direction hits full saturation, which comfortably covers this
   league's actual spread (its Ballparks factors run from the low 80s to
   the high 130s) without everything crowding into the pale middle. */
const pfColor = (v, lowerBetter) => {
  let p = 50 + (v-100)*(50/30);
  if(lowerBetter) p = 100-p;
  return svColor(Math.max(0, Math.min(100, p)));
};
const pfPill = (v, lowerBetter) => v==null ? '—'
  : `<span class="pfpill" style="background:${pfColor(v, lowerBetter)}">${v}</span>`;
...
function svPanel(title, metrics, subject, pool, unqualified, cat){
  const rows = metrics.map(([lab,fn,fmt,low], i)=>{
    const p = svPct(pool.map(fn).filter(isFinite), fn(subject), low);
    if(p==null) return '';
    const idxAttr = cat ? ` data-cat="${cat}" data-idx="${i}"` : '';
    return `<div class="svrow${unqualified?' svunq':''}"${idxAttr}><span class="svlab">${lab}</span>
  ...
  return rows ? `<div class="svpanel${unqualified?' svpanel-unq':''}"${catAttr}><h4>${titleHTML}</h4>${rows}</div>` : '';
}
function savantInner(pl){
  ...
  const panels = (batOK ? svPanel(`Batting · vs ${qb.length}`, SV_BAT, row, qb, false, 'bat')
      : batShow ? svPanel(`Batting · vs ${qb.length}`, SV_BAT, row, qb, true, 'bat') : '')
    + (pitOK ? svPanel(`Pitching · vs ${qp.length}`, SV_PIT, row, qp, false, 'pit')
      : pitShow ? svPanel(`Pitching · vs ${qp.length}`, SV_PIT, row, qp, true, 'pit') : '');
  if(!panels){
...
/* the years a Play Progression run would step through — same qualification
...
function savantCard(pl){
  ...
  return `<section class="savant" id="savantCard">
    ...
    <div id="svBody">${savantInner(pl)}</div></section>`;
}
/* only the chip-driven body needs rewiring after every re-render — the
   header's Play/Card buttons live outside #svBody and are never destroyed,
   so wiring them more than once (e.g. after every Play Progression run)
   would double-fire their click handlers */
function wireSavantBody(pl){
  const body = document.getElementById('svBody');
  if(!body) return;
  body.querySelectorAll('.svchips button').forEach(b=>b.addEventListener('click',()=>{
...
function wireSavant(pl){
  ...
  if(cardBtn) cardBtn.addEventListener('click', ()=>downloadPlayerCard(pl, svYear));
...
function updateSavantValues(pl, y){
  ...
  const applyRows = (metrics, cat, pool, show) => {
    metrics.forEach(([lab,fn,fmt,low], i)=>{
      ...
      if(p==null){
        dot.style.opacity = '.3'; dot.style.left = '50%';
        dot.style.background = 'rgba(255,255,255,.25)'; dot.textContent = '';
        val.textContent = '—';
      } else {
        dot.style.opacity = '1'; dot.style.left = p+'%';
        dot.style.background = svColor(p); dot.textContent = p;
...
async function playPercentiles(pl){
  if(svPlaying) return;
  const years = svPlayableYears(pl);
...
async function buildPlayerCard(pl, year){
  ...
  /* the header's top-right is empty (headshot/logo/name/team all sit left),
     so the league badge gets real room to read as a seal there instead of
     being squeezed down to a tiny icon next to the wordmark. */
  if(brandLogoImg){
    const BLS = 130;
    const bx = CARD_W-PAD-BLS, by = 34;
    ctx.drawImage(brandLogoImg, bx, by, BLS, BLS);
  }
...
function franchiseTimeline(){
  ...
  const rows = FRANCHISE_TIMELINE.map((fr,i)=>{
    ...
    const nameCell = `<div class="tlg-cell tlg-name" style="grid-row:${r};grid-column:1">
      ${logo?logoIcon(logo,'','tl-logo'):'<span class="tl-logo tl-logo-blank"></span>'}${nameEl}</div>`;
...
function renderTeams(){
  ...
  const sumRows = FRANCHISE_SUMMARY.map(d=>{
    ...
    return `<tr>
      <td class="lft">${logoIcon(logo,'','fdotlogo')}${link}</td>
...
function renderStandings(){
  ...
  const fldT = statTable('Team Fielding · '+standYear+' · '+phaseLabel, [
    {l:'Team',lft:1,f:d=>nk(d.name)},
  ...
  app.innerHTML = `
    ...
    Team Batting, Team Pitching and Team Fielding are the sum of that season's roster lines for the selected phase
    above (fielding only where per-game data has been recorded — 2020 on);
...
const llist = (t, items, fmt, low) => {
  ...
  const logos = it => logoIcon(it.logo2 || it.logo, '');
  return `<div class="llist"><h4>${t}</h4><ol>${items.map(it=>
...
function renderLeaders(){
  ...
  const fullStatsHTML = () => {
    ...
    const fullCell = (r,k,t) => {
      if(k==='team') return `<span class="tmcell">${logoIcon(r.logo2 || r.logo, '')}${r.teamLabel||''}</span>`;
      const v = r[k];
...
function teamAccolades(name){
  ...
  return `<section class="stat accolades"><h3>Team Accolades</h3>
    ...
    ${hasAwards ? `<button class="pname" data-tawards="${esc(name)}">Awards →</button>` : ''}
  </section>`;
}
...
function renderRecords(){
  ...
  const teamStreakList = (title, items) => {
    ...
    return `<div class="llist"><h4>${title}</h4><ol>${items.map(it=>{
      ...
      return `<li style="--w:${w(it.v).toFixed(1)}%">${logoIcon(logo, '')}<span class="ln">${
...
/* ---- Park Factors: how much each field's offensive environment differs
   from that year's league average, blended across every year it's been
   used — same era-normalized "actual vs. expected" ratio OPS+/ERA+ already
   use to compare a season to its own year rather than an absolute cross-era
   number, since this league's scoring swung roughly 2.5x across its history
   (2017's league-wide runs/game was double 2024's). Runs PF is computed from the official
   recorded line score (away.R/home.R) — the same authoritative result box
   scores note elsewhere — across every Regular season game; every other
   factor needs individual box-score bat lines (2020 on) and is computed
   only from games that have them. Modeled on Baseball Savant's own Park
   Factors leaderboard, minus the columns that need batted-ball tracking
   (xwOBA, Hard Hit%, etc.) this league's box scores don't have, and using
   a simpler park-vs-league-average method rather than Savant's actual
   same-player-elsewhere matched comparison — worth being upfront about,
   since it's a real methodological difference, not just fewer columns. */
const PARK_MIN_G = 15;
function weightedRatioFactor(byYear, leagueByYear, numKey, denKey){
  let actualNum=0, expectedNum=0;
  Object.keys(byYear).forEach(y=>{
    const lg = leagueByYear[y]; if(!lg || !lg[denKey]) return;
    const rate = lg[numKey]/lg[denKey];
    actualNum += byYear[y][numKey];
    expectedNum += rate*byYear[y][denKey];
  });
  return expectedNum ? Math.round(100*actualNum/expectedNum) : null;
}
function computeParkFactors(){
  const parkYear = {}, leagueYear = {};
  const touch = (map, key, y) => (map[key] = map[key] || {})[y] = (map[key][y] || {
    games:0, runs:0, boxGames:0, h:0, hr:0, bb:0, so:0, obpNum:0, obpDen:0});
  GIDS.forEach(gid=>{
    const g = GAMES[gid];
    if(g.phase!=='Regular' && g.phase!=='Playoffs') return;
    const y = +g.date.slice(0,4);
    const loc = g.loc && g.loc!=='TBA' ? g.loc : null;
    const runs = g.away.R + g.home.R;
    touch(leagueYear, 'league', y).games++; leagueYear.league[y].runs += runs;
    if(loc){ touch(parkYear, loc, y).games++; parkYear[loc][y].runs += runs; }
    const hasBox = g.away.bat.length>=3 && g.home.bat.length>=3;
    if(!hasBox) return;
    const bat = [...g.away.bat, ...g.home.bat];
    const h = bat.reduce((s,b)=>s+(b.h||0),0), hr = bat.reduce((s,b)=>s+(b.hr||0),0);
    const bb = bat.reduce((s,b)=>s+(b.bb||0),0), so = bat.reduce((s,b)=>s+(b.k||0),0);
    const hbp = bat.reduce((s,b)=>s+(b.hbp||0),0), ab = bat.reduce((s,b)=>s+(b.ab||0),0);
    const obpNum = h+bb+hbp, obpDen = ab+bb+hbp;
    const apply = row => { row.boxGames++; row.h+=h; row.hr+=hr; row.bb+=bb; row.so+=so;
      row.obpNum+=obpNum; row.obpDen+=obpDen; };
    apply(leagueYear.league[y]);
    if(loc) apply(parkYear[loc][y]);
  });
  const factors = [
    ['runFactor', 'runs', 'games', false],
    ['hFactor', 'h', 'boxGames', false],
    ['hrFactor', 'hr', 'boxGames', false],
    ['bbFactor', 'bb', 'boxGames', false],
    ['soFactor', 'so', 'boxGames', true],
  ];
  return Object.keys(parkYear).map(loc=>{
    const byYear = parkYear[loc];
    const years = Object.keys(byYear).map(Number).sort((a,b)=>a-b);
    const games = years.reduce((s,y)=>s+byYear[y].games, 0);
    const boxGames = years.reduce((s,y)=>s+byYear[y].boxGames, 0);
    const row = {loc, games, boxGames, span: years.length>1?`${years[0]}–${years[years.length-1]}`:`${years[0]}`};
    factors.forEach(([key, numKey, denKey])=>{
      row[key] = boxGames>=PARK_MIN_G || denKey==='games'
        ? weightedRatioFactor(byYear, leagueYear.league, numKey, denKey) : null;
    });
    row.obpFactor = boxGames>=PARK_MIN_G ? weightedRatioFactor(byYear, leagueYear.league, 'obpNum', 'obpDen') : null;
    return row;
  }).filter(r=>r.games>=PARK_MIN_G && r.runFactor!=null).sort((a,b)=>b.runFactor-a.runFactor);
}
const PARK_COLS = [
  {l:'Field',lft:1,f:d=>esc(d.loc)},
  {l:'G',f:d=>d.games},
  {l:'Years',f:d=>d.span},
  {l:'Runs PF',m:1,f:d=>pfPill(d.runFactor)},
  {l:'OBP PF',m:1,f:d=>pfPill(d.obpFactor)},
  {l:'H PF',m:1,f:d=>pfPill(d.hFactor)},
  {l:'HR PF',m:1,f:d=>pfPill(d.hrFactor)},
  {l:'BB PF',m:1,f:d=>pfPill(d.bbFactor)},
  {l:'SO PF',m:1,f:d=>pfPill(d.soFactor, true)},
];
function parkFactorsHTML(){
  const rows = computeParkFactors();
  if(!rows.length) return '<p class="empty">Not enough located games on record yet.</p>';
  const leagueRow = {loc:'League Average', games:'', span:'', boxGames:'',
  ...
  return `<p class="pmeta">How much each field's offensive environment differs from that year's league
    ...
    individual box scores (2020 on) and needs ${PARK_MIN_G}+ such games on its own. Needs ${PARK_MIN_G}+
...
function renderGames(){
  ...
  const viewToggle = `<div class="segs" role="group" aria-label="View">
    <button data-gv="list" aria-pressed="${gamesView==='list'}">Games</button>
    <button data-gv="parks" aria-pressed="${gamesView==='parks'}">Park Factors</button></div>`;
  if(gamesView==='parks'){
    app.innerHTML = `${viewToggle}${parkFactorsHTML()}`;
    app.querySelectorAll('[data-gv]').forEach(b=>b.addEventListener('click',()=>{ gamesView=b.dataset.gv; renderGames(); }));
    return;
  }
  ...
  app.innerHTML=`${viewToggle}${chips}
...
function tnick(raw, year){
  ...
  return raw.split(/[/,]/).map(part=>{
    ...
    return `<button class="pname" data-t="${esc(full)}">${logoIcon(logo, '', 'tlogo-mini')}${esc(label)}</button>`;
  }).join(' / ');
}
const wireHonors = () => {
  app.querySelectorAll('.pname[data-p]').forEach(b=>b.addEventListener('click',()=>{ location.hash='#/p/'+encodeURIComponent(b.dataset.p); }));
...
function nwlaAwardsSection(){
  ...
  /* one row per honoree (year, tier, player, team) rather than a Year×Tier
  ...
  const allTeamTable = (title, catKey) => {
    ...
    return `<h4>${esc(title)}</h4><div class="tscroll"><table class="detail"><thead><tr>
      <th class="lft">Year</th><th class="lft">Tier</th><th class="lft">Player</th><th class="lft">Team</th>
...
const NON_PLAYER_AWARDS = new Set(['Sox Trophy']);
/* every award tied to a franchise, resolved through the same team-
   abbreviation machinery tnick() uses on the main Awards page. Individual
   player awards are paired positionally with the winner list on the old
   (2012-2017) multi-team/multi-winner Golden Hands/Silver Slugger rows,
   falling back to the raw winner string if the two lists don't line up
   1:1 (a few of those old rows have mismatched counts). Team of the Year
   and Game of the Year are team-level (their "winner" is a team name or a
   game description, not a player) but are included here too — Sox Trophy
   stays out since it's just a regular-season-record tiebreaker, not a
   standalone honor. */
function teamAwardEntries(fullName){
  ...
  Object.keys(AWARDS).forEach(y=>{
    AWARDS[y].forEach(r=>{
      ...
      const winnerParts = (r.winner||'').split(/\s*[/,]\s*/);
      teamParts.forEach((tp,i)=>{
...
function renderTeamAwards(name){
  ...
  const entries = teamAwardEntries(name);
  const grouped = {};
  entries.forEach(e=>{ (grouped[e.award] = grouped[e.award] || []).push(e); });
  const gkeys = Object.keys(grouped).sort((a,b)=>{
    const ia=AW_ORDER.indexOf(a), ib=AW_ORDER.indexOf(b);
    return (ia<0?99:ia)-(ib<0?99:ib) || a.localeCompare(b);
  });
...
function buildTicker(){
  ...
  const tkLogo = (tm,y) => logoIcon(teamLogoForYear(tm,y), '', 'tk-logo');
...
const qualifyText = n => `${n} player${n===1?'':'s'} ${n===1?'qualifies':'qualify'}`;
/* once every square's been attempted, each cell can reveal its whole
   qualifying pool (not just the count) — poolNames is only computed by
   the caller once the grid is done, so there's nothing to spoil mid-game */
function gridPoolReveal(poolNames){
  return `<details class="gpoolreveal"><summary>${qualifyText(poolNames.length)} →</summary>
    <ul class="gpoollist">${poolNames.map(n=>`<li><button class="pname" data-p="${esc(n)}">${esc(n)}</button></li>`).join('')}</ul></details>`;
}
function gridCellHTML(k, ans, poolNames){
  if(ans){
...
function renderGrid(){
  ...
  const rowsHTML = puzzle.rows.map((rc,ri)=>{
    const cells = puzzle.cols.map((cc,ci)=>{
      const k = ri+','+ci;
      const poolNames = done ? [...rc.set].filter(n=>cc.set.has(n)).sort((a,b)=>nameLast(a).localeCompare(nameLast(b))) : null;
      return `<td class="gcell${puzzle.answers[k]?(puzzle.answers[k].correct?' correct':' wrong'):''}">${gridCellHTML(k, puzzle.answers[k], poolNames)}</td>`;
    }).join('');
...
/* teamOPS+/teamERA+ (not plain OPS/ERA) drive the run projection — the
   draft pulls player-seasons from 2017 through today, and this league's
   own scoring environment has shifted a lot over that span (more teams,
   different competitive levels), so a season judged only against a flat
   all-time average would over- or under-rate it depending purely on what
   year it happened to come from. opsPlusFor()/eraPlusFor() already solve
   this everywhere else on the site by weighting each contributing year's
   own league average by that pick's PA/innings — reused as-is here. */
function b0Simulate(picks, pitcherNames, rng){
  const teamBat = sumRows(picks.map(p=>p.regLine));
  const pitPicks = picks.filter(p=>pitcherNames.includes(p.name));
  const teamPit = sumRows(pitPicks.map(p=>p.regLine));
  const teamOPSPlus = opsPlusFor(teamBat, picks.map(p=>({year:p.year, pa:p.regLine.PA})));
  const teamERAPlus = eraPlusFor(teamPit, pitPicks.map(p=>({year:p.year, outs:p.regLine.IPouts})));
  const lg = b0League();
  const pool = b0RunsPool();
  const projRPG = isFinite(teamOPSPlus) ? lg.rpg*(teamOPSPlus/100) : lg.rpg;
  const projRA = (isFinite(teamERAPlus) && teamERAPlus>0) ? lg.rpg*(100/teamERAPlus) : lg.rpg;
  ...
  return {teamOPSPlus, teamERAPlus, projRPG, projRA, winPct, wins, losses: B0_GAMES-wins, log};
}
  ...
  if(puzzle.locked && puzzle.result){
    ...
    bodyHTML = `<div class="b0result" style="--tier:${statpadTier(null, r.wins===B0_GAMES).color}">
      ...
      <p class="pmeta">A simulated ${B0_GAMES}-game season, one game at a time, chasing a perfect ${B0_GAMES}-0 —
        this team's combined ${isFinite(r.teamOPSPlus)?r.teamOPSPlus:'—'} OPS+ (all 5) projects to about
        ${r.projRPG.toFixed(1)} runs a game scored, and its two Pitcher Round picks' combined
        ${isFinite(r.teamERAPlus)?r.teamERAPlus:'—'} ERA+ projects to about ${r.projRA.toFixed(1)} runs a game
        allowed, against a league that's averaged ${lg.rpg.toFixed(1)} runs a game in real games — an expected
        win rate of ${rate(r.winPct)}. OPS+ and ERA+ each measure a pick against that year's own league average
        (100 = average that year), so a great season from an early, lower-scoring year and a great season from a
        later, higher-scoring one count the same instead of the later one just looking better on paper. Each
        game's score is a real BWB team-game run total pulled off the league's own board and scaled to this rate,
        not a textbook bell curve, so blowouts and shutouts show up as often as they actually have around here.
        Every rate here is also already regressed toward league average by how much each player actually played
        that season.</p>
...
const NAV_HREF = {
  ...
  tawards: v => '#/t/'+encodeURIComponent(v)+'/awards',
  g: v => '#/g/'+v,
